...
// 50 tasks spread across Israel (lon, lat) — ST_MakePoint expects (longitude, latitude)
const taskSeedData = [
  // ══════════ TEL AVIV (12 tasks) ══════════
  ...
  // ══════════ HAIFA (8 tasks — kept from original) ══════════
  ...
  { title: 'תלייה של טלוויזיה 65 אינץ על הקיר', description: 'טלוויזיה Samsung 65" צריכה תלייה. יש ברגים, צריך מישהו עם כלים.', category: Category.MOUNTING, suggested_price: 200, general_location_name: 'הדר הכרמל, חיפה', exact_address: 'רחוב ביאליק 12, הדר הכרמל', lon: 35.0040, lat: 32.8180 },
  ...
  { title: 'העברת תכולת חדר מ-A ל-B', description: 'העברה של רהיטים מחדר אחד לשני באותו בניין. ארון, מיטה, שולחן.', category: Category.MOVING, suggested_price: 200, general_location_name: 'שכונה ג, באר שבע', exact_address: "רחוב ז'בוטינסקי 5, באר שבע", lon: 34.7990, lat: 31.2540 },

  // ══════════ NETANYA / HERZLIYA / SHARON (6 tasks) ══════════
  ...
  { title: 'החלפת דלת כניסה', description: 'דלת כניסה ישנה לדירה. צריך הסרה והתקנת דלת חדשה כולל מנעול.', category: Category.ASSEMBLY, suggested_price: 800, general_location_name: 'רמת השרון', exact_address: 'רחוב סוקולוב 20, רמת השרון', lon: 34.8390, lat: 32.1460 },
  ...
  { title: 'הובלת ריהוט משד אשדוד', description: 'ספה, שולחן אוכל ו-6 כיסאות. הובלה מאשדוד לבאר שבע.', category: Category.MOVING, suggested_price: 700, general_location_name: 'אשדוד', exact_address: 'שדרות הנשיאים 12, אשדוד', lon: 34.6500, lat: 31.8010 },
...
async function main() {
  ...
  // Distribute tasks across requesters (50 tasks, round-robin across 3 requesters)
  const reqPool = [requester1.id, requester2.id, requester3.id];
  const requesterIds = Array.from({ length: taskSeedData.length }, (_, i) => reqPool[i % 3]);
  ...
  console.log(`✓ Seed complete — 6 users, ${taskIds.length} tasks, bids created`);
}
